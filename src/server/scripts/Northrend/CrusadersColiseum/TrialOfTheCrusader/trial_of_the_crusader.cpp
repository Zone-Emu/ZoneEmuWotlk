    ...
    public:
        ...
        struct npc_announcer_toc10AI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                if (Creature* horde = GetClosestCreatureWithEntry(me, NPC_THRALL, 300.0f))
                    horde->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
...
class boss_lich_king_toc : public CreatureScript
{
    public:
        ...
        struct boss_lich_king_tocAI : public ScriptedAI
        {
            ...
            {
                if (!instance) return;
                if (instance->GetData(TYPE_EVENT_NPC) != NPC_LICH_KING_1) return;

                uiUpdateTimer = instance->GetData(TYPE_EVENT_TIMER);
                if (uiUpdateTimer <= uiDiff)
                {
                ...
                } else uiUpdateTimer -= uiDiff;
                instance->SetData(TYPE_EVENT_TIMER, uiUpdateTimer);
            }
        };
                ...
                if (uiUpdateTimer <= uiDiff)
                {
                    switch (instance->GetData(TYPE_EVENT))
                    {
                        ...
                        case 1130:
                            ...
                            {
                                uiTriggerGUID = trigger->GetGUID();
                                trigger->SetFloatValue(OBJECT_FIELD_SCALE_X, 2.0f);
                        ...
                        case 1134:
                            ...
                            if (Creature* portal = me->SummonCreature(NPC_WILFRED_PORTAL, ToCCommonLoc[1].GetPositionX(), ToCCommonLoc[1].GetPositionY(), ToCCommonLoc[1].GetPositionZ(), 4.71239f, TEMPSUMMON_MANUAL_DESPAWN))
                            {
                                ...
                                portal->SetFloatValue(OBJECT_FIELD_SCALE_X, 2.0f);
                                portal->CastSpell(portal, SPELL_WILFRED_PORTAL, false);
                                uiPortalGUID = portal->GetGUID();
                            }
                            uiUpdateTimer = 4000;
                        ...
                        case 1142:
                            ...
                            if (Creature* trigger = Unit::GetCreature(*me, uiTriggerGUID))
                                trigger->DespawnOrUnsummon();
                            if (Creature* portal = Unit::GetCreature(*me, uiPortalGUID))
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                if (uiUpdateTimer <= uiDiff)
                {
                    switch (instance->GetData(TYPE_EVENT))
                    {
                        ...
                        //Summoning crusaders
                        case 3091:
                            if (Creature* championController = me->SummonCreature(NPC_CHAMPIONS_CONTROLLER, ToCCommonLoc[1]))
                                championController->AI()->SetData(0, HORDE);
                            uiUpdateTimer = 3000;
